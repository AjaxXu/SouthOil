...
(function ($) {
    ...
    function initLayout(target) {
        ...
        var loginDiv = "<div id='login-div' style='font-size: 20px;color: #3e42ff;text-align: right;margin-right: 60px;display: none;'><span id='loginUser'></span>,欢迎您!</div>";
        center.append(loginDiv);
        var div = "<div class='app-title'>" + opts.title + "</div>"
        center.append(div);
    ...
    function initApp(target, apps) {
        ...
        var height = wall.width();
        var liLength = relSize * appSize;
        var leftWidth = (height - liLength - 60) / 2;
        //初始值
        var line = 1,
            col = 1,
            top = 100,
    ...
    function createMenu(target, menus, opt) {
        var opts = $.data(target, 'app').options;
        var menuDiv = $('<div style="width:120px;"></div>').appendTo('body');
    ...
    function openApp(target, options) {
        ...
        if (appOpt.href && /^http/i.test(appOpt.href) || appOpt.iframe) {
            ...
            // appWindow.load(appOpt.href,function (item) {
            // 	console.log("div loaded perfomance");
            //     opt.onElementLoaded.call(item);
            // });
    ...
    $.fn.app.defaults = {
        ...
        onElementLoaded: function (item) {
        }, //开始菜单点击事件
        onBeforeClose: function (item) {
        },
        iframeOpen: false,
        loadUrl: { //远程数据加载路径
